...
WITH combined AS (
    SELECT Player_A AS personnalite, Resultat_P_A AS resultat FROM {{ ref('stg_parquets') }}
    UNION ALL
    SELECT Player_B AS personnalite, Resultat_P_B AS resultat FROM {{ ref('stg_parquets') }}
)
SELECT
